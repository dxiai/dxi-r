...
Um an eingebettete Werte zu gelangen, müssen wir sie zuerst "ausbetten". Dazu verwenden wir die Funktion `unnest()`. Mit dieser Funktion werden eingebette Listen in einen Vektor entpackt ([@sec-hierarchisieren]). 
...
Gelegentlich sind relevante Informationen semi-strukturiert in Zeichenketten verborgen. Ein typisches Beispiel dafür sind sog. Hash-Tags in Beiträgen in den sozialen Medien. Für die Analyse solcher Daten müssen diese Werte zuerst aus den umgebenenden Zeichenketten extrahiert werden. Das leistet die Funktion `str_extract()` für einen einzelnen eingebetteten Wert bzw. `str_extract_all()` für alle eingebettete Werte.  Beide Funktionen erwarten zwei Parameter: einen Zeichenkettenvektor als ersten Parameter und einen regulären Ausdruck, für die zu extrahierenden Muster.

Die Funktion `str_extract()` hat immer eine einzele Zeichenkette als Ergebnis. Wenn es in der Zeichenkette mehrere Vorkommnisse des Musters gibt, wird das erste Auftreten zurückgegeben. Gibt es keinen Treffer, ist das Ergebnis der Wert `NA.

Die FUnktion `str_extract_all() hat immer eine Liste mit Zeichenketten als Ergebnis. Existiert das Muster in der Zeichenkette nicht, wird eine leere Ergebnisliste geliefert. 


## Suchen und Ersetzen

Eine wichtige Operation beim Bereinigen von Daten ist das Suchen und Ersetzen. Mit dieser Operation lassen sich Schreibweisen vereinheitlichen oder Tippfehler korrigieren. Das Suchen und Ersetzen ist zwingend, wenn Eingaben von Menschen ausgewertet werden sollen. Dazu dienen die beiden Funktionen `str_replace()` und `str_replace_all()`. Bei Texteingaben kommt in der Regel die Funktion `str_replace_all()` zur Anwendung, weil diese Sicherstellt, dass alle Abweichungen in der gleichen Zeichenkette korrigiert werden. Die Funktion `str_replace()` wird nur bei kurzen Zeichenketten verwendet, bei dem sichergestellt ist, dass die Abweichung immer genau einmal vorkommen kann. Das ist typisch für automatisch generierte Daten. 

Beide Funktionen haben drei Parameter: 

1. Die Zeichenkette bzw. ein Zeichenkettenvektor.
2. Das Suchmuster 
3. Die Ersetzung für das Suchmuster

Ein Vorteil der Funktion `str_replace_all()` ist ausserdem, dass mehrere Muster in einem Arbeitsschritt gesucht und überall ersetzt werden können. Dazu werden die Suchmuster und Ersetzungen als benannte Vektor als zweiter Parameter übergeben. In diesem Fall entfällt der dritte Parameter. 

::: {#exm-suchen-ersetzen}
## Mehrfaches Suchen und Ersetzen von Texteingaben

```{r}
# Schreibweisen von WhatsApp korrigieren

tibble(
    texte = c(
        "WhatsApp  SBB App  Telegram  ZKB Twint  ZKB Banking",
        "Whatsapp  Snapchat  Kalender  Wetter  Wecker", 
        "Whats App  Zatoo  Google Maps  Photomath  UBS Banking"
    )
) |>
    mutate(
        texte = str_replace_all(texte, c(
            "Whatsapp" = "WhatsApp",
            "Whats App" = "WhatsApp"
        ))
    )
...
Gelegentlich ist die Position einer Zeichenkette von Bedeutung. Mit den Funktionen `str_locate()` und `str_locate_all()` lassen sich die Positionen einer Zeichenkette in einer anderen Zeichenkette finden. Das Ergebnis ist eine benannte Liste mit der Start- und Endposition der gesuchten Zeichenkette. 
...
```{r}
texte = c("Daten", "Information", "ZHAW")

texte |> str_locate("at")
```
:::

In einem Datenrahmen ist das Ergebnis eingebettet und muss anschliessend ausgebettet werden.

::: {#exm-str-locate-df}
## Die Teilzeichenkette "at" finden
```{r}
tibble(
    texte = c("Daten", "Information", "ZHAW")
) |> 
    mutate(
        positionen = texte |> str_locate("at")
    ) |>
    unnest()
```
:::
...
## Mustererkennung {#sec-regular-expressions}
